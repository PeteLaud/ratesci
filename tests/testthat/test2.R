...
test_that("p-values consistent with paired confidence interval", {
  expect_equal(
    ...
    (((sapply(1:dim(combos)[1],function(i) pairbinci(x=combos[i,],contrast="RD")$pval[,"pval.right"] < 0.025 ))))
  )
})
for (contrast in c("RR","OR")) {
  test_that("p-values consistent with paired confidence interval", {
    expect_equal(
      (((sapply(1:dim(combos)[1],function(i) pairbinci(x=combos[i,],contrast=contrast,method.OR='SCAS')$estimates[1] > 1)))),
      (((sapply(1:dim(combos)[1],function(i) pairbinci(x=combos[i,],contrast=contrast,method.OR='SCAS')$pval[,"pval.right"] < 0.025 ))))
    )
  })
}  
